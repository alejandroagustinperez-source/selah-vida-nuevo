...
app.post('/api/contact', async (req, res) => {
  try {
    ...
    if (!resend) {
      console.warn('RESEND_API_KEY not set — logging contact form');
    ...
    const { data, error } = await resend.emails.send({
      ...
      html: `
        <div style="font-family: Inter, sans-serif; max-width: 600px; margin:0 auto; padding:32px 24px; background:#FAF6EF; color:#1a3a4a;">
          ...
          <div style="margin-top:16px; padding:16px; background:#fff; border-radius:8px; border:1px solid #e0d6c8; font-size:14px; line-height:1.7; white-space:pre-wrap;">${message}</div>
